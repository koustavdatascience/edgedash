...
def inject_css() -> None:
    ...
    header[data-testid="stHeader"] { background:transparent; height:0; } [data-testid="stToolbar"] { display:none; } #MainMenu { visibility:hidden; }
    ...
    .ed-nav { position:relative; z-index:3; margin:0 0 20px; padding:0 18px; height:58px; border:1px solid rgba(255,255,255,.28); border-radius:14px; background:rgba(255,255,255,.72); backdrop-filter:blur(16px); box-shadow:0 8px 28px rgba(16,24,40,.08); }
    .brand-mark { color:#fff; background:#171717; }
    .hero { position:relative; overflow:hidden; margin:0; height:min(68vh,620px); min-height:0; border-radius:20px; box-shadow:0 20px 60px rgba(16,24,40,.12); display:flex; justify-content:flex-end; background:#0e1b35; }
